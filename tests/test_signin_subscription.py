...
class DummyCursor:
    def __init__(
        self, active=True, expired=False, has_subscription=True, usertype="Parent"
    ):
        self.active = active
    ...
    def fetchone(self):
        ...
        if self.calls == 0:
            ...
            return (1, "user", b"hash", self.usertype)
...
class DummyConnection:
    def __init__(
        ...
        self.cursor_obj = DummyCursor(active, expired, has_subscription, usertype)
...
def test_student_signin_requires_active_subscription(client):
    conn = DummyConnection(active=False, usertype="Student")
    with patch('auth.get_db_connection', return_value=conn), \
         patch('extensions.bcrypt.check_password_hash', return_value=True):
        resp = client.post('/api/signin', json={'username': 'user', 'password': 'pw'})
